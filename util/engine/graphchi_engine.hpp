...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    public:
         ...
         */
        void run(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int _niters, int start_interval = -1) {
            ...
            for(iter=0; iter < niters; iter++) {
                ...
                if((start_interval != -1) && (iter == 0))
                    logstream(LOG_INFO) << "Initial interval set to " << start_interval << std::endl;
                /* Interval loop */
                for(int interval_idx=0; interval_idx < nshards; ++interval_idx) {
                    if((start_interval != -1) && (iter == 0))
                        exec_interval = start_interval;
                    else
