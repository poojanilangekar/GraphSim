...
namespace graphchi {
     ...
     */
    template <typename EdgeDataType, typename FinalEdgeDataType>
    int convert(std::string basefilename, std::string nshards_string, std::string file_type_str = std::string()) {
        sharder<EdgeDataType, FinalEdgeDataType> sharderobj(basefilename);
        
        if(file_type_str.empty())
            file_type_str = get_option_string_interactive("filetype", "edgelist, adjlist, binedgelist, metis");
        ...
        logstream(LOG_DEBUG)<<"Creating shards from file "<<basefilename<<" of filetype "<<file_type_str;
        /* Start preprocessing */
    ...
    template <typename EdgeDataType>
    int convert_if_notexists(std::string basefilename, std::string nshards_string, bool &didexist, std::string file_type_str = std::string()) {
        int nshards;
        ...
        logstream(LOG_INFO) << "(Edge-value size: " << sizeof(EdgeDataType) << ")" << std::endl;
        logstream(LOG_INFO) << "Will try create them now..." << std::endl;
        nshards = convert<EdgeDataType, EdgeDataType>(basefilename, nshards_string,file_type_str);
        return nshards;
    }
    ...
    int convert_if_notexists(std::string basefilename, std::string nshards_string, bool &didexist, std::string file_type_str = std::string()) {
        ...
        logstream(LOG_INFO) << "Will try create them now..." << std::endl;
        nshards = convert<EdgeDataType, FinalEdgeType>(basefilename, nshards_string,file_type_str);
        return nshards;
    }
    
    template <typename EdgeDataType, typename FinalEdgeType>
    int convert_if_notexists(std::string basefilename, std::string nshards_string,std::string file_type_str = std::string() ) {
        bool b;
        return convert_if_notexists<EdgeDataType, FinalEdgeType>(basefilename, nshards_string, b, file_type_str);
    }
    
    template <typename EdgeDataType>
    int convert_if_notexists(std::string basefilename, std::string nshards_string, std::string file_type_str = std::string()) {
        bool b;
        return convert_if_notexists<EdgeDataType, EdgeDataType>(basefilename, nshards_string, b,file_type_str);
    }
